...
repeat
	...
	for i=1, opt.length do
		...
		for pidx=1,probs:numel() do
--			if (probs[pidx]<0) then
--				print (pidx ", is negative.")
--			end
--			if (probs[pidx] ~= probs[pidx]) then
--				print (pidx ", is NaN");
--			end
			if (probs[pidx] > 0.999) then
				probs[pidx]=probs[pidx]-0.001;
			end
		end
--		print(probs:float())
                prev_char = torch.multinomial(probs:float(), 2, false):float()
--		print("Multi worked fine")
