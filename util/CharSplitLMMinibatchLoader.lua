...
-- Modified by ADF 4/12/16 to preprocess vocab and data separately
...
function CharSplitLMMinibatchLoader.create(data_dir, batch_size, seq_length, split_fractions, word_level, threshold)
    ...
    local run_dprepro = false
    if not (path.exists(vocab_file)) then
        -- prepro files do not exist, generate them
        print('vocab.t7 does not exist. Running preprocessing...')
        run_prepro = true
    else
        -- check if the input file was modified since last time we 
        -- ran the prepro. if so, we have to rerun the preprocessing
	print('Vocab.t7 exists')
	    ...
	    print('Vocab.t7 is up to date')
	    if not (path.exists(tensor_file)) then
		run_dprepro=true
		print('No data file, rewriting')
	    else
    ...
    if run_prepro then
        ...
        CharSplitLMMinibatchLoader.text_to_vocab(word_level, threshold, input_file, vocab_file, tensor_file)
        CharSplitLMMinibatchLoader.text_to_tensor(word_level, threshold, input_file, vocab_file, tensor_file)
    else
	if run_dprepro then
	    print('Processing data only, with existing vocab file')
            CharSplitLMMinibatchLoader.text_to_tensor(word_level, threshold, input_file, vocab_file, tensor_file)
	else
		print('No need to reprocess data file')
	end
    end
...
-- *** STATIC method ***
function CharSplitLMMinibatchLoader.text_to_vocab(word_level, threshold, in_textfile, out_vocabfile, out_tensorfile)
    ...
    local numtokens = 0 
    -- make it easier for capitals to get into the vocab
    local capmult=1
    local basemult=1
    for token in CharSplitLMMinibatchLoader.tokens(rawdata, word_level) do
	local countsize=basemult
--	Remove trailing ASCII 13
	if (string.byte(token,-1)==13 and token:len()>1) then
--	   print("Dethirteening " .. token)
--	   print("Length ",token:len())
	   token=token:sub(1,-2)
	end
	if token:len() > 0 then
		if token ~= token:lower() then
	   		countsize=capmult
		end
        	if not unordered[token] then 
            		unordered[token] = countsize
        	else
            		unordered[token] = unordered[token] + countsize
        	end
...
function CharSplitLMMinibatchLoader.text_to_tensor(word_level, threshold, in_textfile, in_vocabfile, out_tensorfile)
    local timer = torch.Timer()
    print('loading text file...')
    local f = torch.DiskFile(in_textfile)
    local rawdata = f:readString('*a') -- NOTE: this reads the whole file at once
    ...
    vocab_mapping=torch.load(in_vocabfile)
...
--	Remove trailing ASCII 13
	if (string.byte(token,-1)==13 and token:len()>1) then
--	  	print("Tensor Count Dethirteening " .. token)
--	   	print("Length ",token:len())
	   	token=token:sub(1,-2)
	end
        numtokens = numtokens + 1
...
--	Remove trailing ASCII 13
		if (string.byte(token,-1)==13 and token:len()>1) then
--	   		print("Tensor Read Dethirteening " .. token)
--	   		print("Length ",token:len())
	   		token=token:sub(1,-2)
		end
	    if token:len()>0 then
