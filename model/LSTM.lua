...
function LSTM.lstm(input_size, rnn_size, n, dropout, recurrent_dropout, embedding, num_fixed)
  ...
  for L = 1,n do
	...
	-- replace uniform with orthogonal
	if (ortho_init == 1) then
    	   i2h = nn.Linear(input_size_L, 4 * rnn_size):init('weight', nninit.orthogonal)(x):annotate{name='i2h_'..L}
    	   h2h = nn.Linear(rnn_size, 4 * rnn_size):init('weight', nninit.orthogonal)(prev_h):annotate{name='h2h_'..L}
	else
  ...
  if (ortho_init == 1) then
    proj = nn.Linear(rnn_size, input_size):init('weight', nninit.orthogonal)(top_h):annotate{name='decoder'}
  else
    proj = nn.Linear(rnn_size, input_size):init('weight', nninit.uniform, -0.08, 0.08)(top_h):annotate{name='decoder'}
  end
